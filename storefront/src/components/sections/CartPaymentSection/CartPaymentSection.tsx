...
import { isDinar, isDoku, isStripe as isStripeFunc, paymentInfoMap } from '../../../lib/constants';
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (!checkActiveSession) {
        if (isDoku(selectedPaymentMethod)) {
          await initiatePaymentSession(cart, {
              ...
              >
                {availablePaymentMethods
                  .filter(paymentMethod => {
                    // Hide Dinar if user is not logged in
                    if (isDinar(paymentMethod.id) && !customer) {
                      return false;
                    }
                    return true;
                  })
